...
import NamesChart from "@/components/charts/names-chart"
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From February 2025 and earlier</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Plumber, Carpenter, Bricklayer, Construction Expert</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(applications.reduce((sum, app) => sum + app.yearsOfExperience, 0) / applications.length).toFixed(1)}{" "}
              years
            </div>
            <p className="text-xs text-muted-foreground">Across all applicants</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Distribution by experience level</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="col-span-1 md:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Number of applications per position</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]">
                <PositionsChart data={applications} />
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Average years of experience per position</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]">
                <div className="h-full flex items-center justify-center">
                  <table className="w-full">
                    ...
                    <tbody>
                      {["Plumber", "Carpenter", "Bricklayer", "Construction Expert"].map((position) => {
                        const positionApps = applications.filter((app) => app.positionAppliedFor === position)
                        const avgExp =
                          positionApps.length > 0
                            ? (
                                positionApps.reduce((sum, app) => sum + app.yearsOfExperience, 0) / positionApps.length
                              ).toFixed(1)
                            : "N/A"
                        return (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Preferred work arrangements by position</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]">
                <div className="h-full flex items-center justify-center">
                  <table className="w-full">
                    ...
                    <tbody>
                      {["Plumber", "Carpenter", "Bricklayer", "Construction Expert"].map((position) => {
                        const positionApps = applications.filter((app) => app.positionAppliedFor === position)
                        const fullTime = positionApps.filter((app) => app.workArrangement === "Full-time").length
                        const partTime = positionApps.filter((app) => app.workArrangement === "Part-time").length
                        const contract = positionApps.filter((app) => app.workArrangement === "Contract").length
                        const freelance = positionApps.filter((app) => app.workArrangement === "Freelance").length
        ...
        <TabsContent value="skills" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="col-span-1 md:col-span-2">
              <CardHeader>
                <CardTitle>Years of Experience</CardTitle>
                <CardDescription>Distribution across experience ranges</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]">
                <ExperienceChart data={applications} />
              </CardContent>
